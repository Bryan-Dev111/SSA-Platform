/**
 * CAR root cause codes: Admin CRUD; GET for CAR Record dropdown (active only unless Admin ?all=1).
 */
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response): Promise<void> => {
    ...
    const list = await prisma.carRootCauseCode.findMany({
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      const created = await prisma.carRootCauseCode.create({
...
router.patch(
  ...
  asyncHandler(async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      const updated = await prisma.carRootCauseCode.update({
        where: { id: req.params.id },
        data,
      });
      res.json(updated);
    } catch {
      res.status(404).json({ error: 'Root cause code not found' });
    }
  })
);
...
router.delete(
  ...
  asyncHandler(async (req: Request, res: Response): Promise<void> => {
    try {
      await prisma.carRootCauseCode.delete({ where: { id: req.params.id } });
      res.status(204).send();
    } catch {
      res.status(404).json({ error: 'Root cause code not found' });
    }
