/**
 * Disposition codes: Admin CRUD; GET for Findings dropdown (active only unless Admin ?all=1).
 */
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response): Promise<void> => {
    ...
    const list = await prisma.dispositionCode.findMany({
...
router.patch(
  ...
  asyncHandler(async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      const updated = await prisma.dispositionCode.update({
        where: { id: req.params.id },
        data,
      });
      res.json(updated);
    } catch {
      res.status(404).json({ error: 'Disposition code not found' });
    }
  })
);
...
router.delete(
  ...
  asyncHandler(async (req: Request, res: Response): Promise<void> => {
    try {
      await prisma.dispositionCode.delete({ where: { id: req.params.id } });
      res.status(204).send();
    } catch {
      res.status(404).json({ error: 'Disposition code not found' });
    }
