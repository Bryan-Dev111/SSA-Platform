/**
 * Sentinel Supplier Assurance — Internal Management calendar: user-added events (subject + date).
 */
...
router.get(
  ...
  asyncHandler(async (_req: Request, res: Response): Promise<void> => {
    const list = await prisma.internalManagementCalendarEvent.findMany({
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response): Promise<void> => {
    ...
    const created = await prisma.internalManagementCalendarEvent.create({
      data: {
        subject: subjectRaw,
        eventDate,
